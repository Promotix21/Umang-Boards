...
/* ============================================================
   UBL Inner-Page Animation Helpers
   Defined here (module scope) so inner pages can use them
   in their own DOMContentLoaded handlers without race issues.
   ============================================================ */
window.UBL = window.UBL || {};

window.UBL.wipeIn = function(selector, opts) {
    if (typeof gsap === 'undefined' || typeof ScrollTrigger === 'undefined') return;
    var o = Object.assign({ delay: 0.1, start: 'top 75%' }, opts);
    gsap.utils.toArray(selector).forEach(function(el) {
        gsap.fromTo(el,
            { opacity: 0, y: 40, clipPath: 'inset(0 0 100% 0)' },
            { opacity: 1, y: 0, clipPath: 'inset(0 0 0% 0)',
              duration: 1, delay: o.delay, ease: 'power3.out',
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function closeMegas() {
        clearTimeout(megaTimer);
        clearTimeout(megaCatTimer);
        clearTimeout(megaL3Timer);
        allMegas.forEach(m => {
            m.classList.remove('active');
            // Reset all internal active states so re-opening starts clean
            m.querySelectorAll('.mega-cat.active').forEach(c => c.classList.remove('active'));
            m.querySelectorAll('.mega-sub-panel.active').forEach(p => p.classList.remove('active'));
            m.querySelectorAll('.mega-l3.active').forEach(l => l.classList.remove('active'));
            m.querySelectorAll('.mega-l4-panel.active').forEach(p => p.classList.remove('active'));
        });
    }
    ...
    /* ============================================
       ...
       Uses hover-intent delay (150ms) to prevent diagonal
       mouse movement from accidentally switching panels
       (fixes 2-column grid menus like Solutions)
       ============================================ */
    let megaCatTimer = null;
    ...
    document.querySelectorAll('.mega-cat').forEach(cat => {
        cat.addEventListener('mouseenter', () => {
            clearTimeout(megaCatTimer);
            // If nothing is active yet (first hover), activate immediately
            const menu = cat.closest('.mega-menu');
            const hasActive = menu && menu.querySelector('.mega-cat.active');
            if (!hasActive) {
                activateMegaCat(cat);
            } else {
                // Delay switch to allow diagonal mouse movement to L3/L4 panel
                megaCatTimer = setTimeout(() => activateMegaCat(cat), 150);
            }
        });
    ...
    /* ============================================
       ...
       Also uses hover-intent delay for same reason
       ============================================ */
    let megaL3Timer = null;
    function activateMegaL3(l3) {
    ...
    document.querySelectorAll('.mega-l3').forEach(l3 => {
        l3.addEventListener('mouseenter', () => {
            clearTimeout(megaL3Timer);
            const panel = l3.closest('.mega-sub-panel');
            const hasActive = panel && panel.querySelector('.mega-l3.active');
            if (!hasActive) {
                activateMegaL3(l3);
            } else {
                megaL3Timer = setTimeout(() => activateMegaL3(l3), 120);
            }
        });
